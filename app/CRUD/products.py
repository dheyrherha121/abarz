...
def create_poduct(product: schema.Product, db: session = Depends(get_db), current_user: int = Depends(security.get_current_user)):
...
def quary_by_one(id: int, db: session = Depends(get_db)):
    product = db.query(model.Products).filter(model.Products.id == id).first()

    if not product:
        raise HTTPException(status_code=status.HTTP_204_NO_CONTENT,
                            detail='there is no post with this post, check again')
    return {product}


@router.delete('/')
def delete_post(id: int, db: session = Depends(get_db), current_user: dict = Depends(security.get_current_user)):
